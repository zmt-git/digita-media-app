...
export default {
  ...
  methods: {
    ...
    uploadFile(client, file, date = new Date().getTime()) {
      ...
      if (
        !that.uploadFileClient ||
        Object.keys(that.uploadFileClient).length === 0
      ) {
      ...
      const type = file.file.type
        .slice(file.file.type.lastIndexOf("/") + 1)
        .toLowerCase();
      ...
      that.uploadFileClient
        .multipartUpload(key, file.file, options)
        .then((res) => {
